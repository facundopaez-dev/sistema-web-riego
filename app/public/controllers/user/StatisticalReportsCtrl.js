	...
	["$scope", "$location", "$route", "StatisticalReportSrv", "ParcelSrv", "ReasonError", "UtilDate", "AccessManager", "ErrorResponseManager", "AuthHeaderManager",
		...
		function ($scope, $location, $route, statisticalReportService, parcelSrv, reasonError, utilDate, accessManager, errorResponseManager, authHeaderManager,
			...
			*/
			...
			function findAll() {
				statisticalReportService.findAll(function (error, data) {
					if (error) {
			...
			$scope.filter = function () {
				...
				controller que pertenecen a una parcela. Por lo tanto, la
				...
				*/
				...
				que el metodo filter de la clase REST StatisticalReportRestServlet
				de la aplicacion del lado servidor, utiliza la fecha desde y la
				fecha hasta en el formato yyyy-MM-dd para recuperar informes
				estadisticos de la base de datos subyacente.
				*/
				...
				statisticalReportService.filter($scope.parcel.name, newDateFrom, newDateUntil, function (error, data) {
					if (error) {
						console.log(error);
						errorResponseManager.checkResponse(error);
						return;
