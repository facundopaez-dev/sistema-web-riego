app.controller(
    "PastDaysReferenceCtrl",
    ["$scope", "$location", "$routeParams", "PastDaysReferenceSrv", "AccessManager", "ErrorResponseManager", "AuthHeaderManager",
        "LogoutManager", "ExpirationManager", "RedirectManager", function ($scope, $location, $params, pastDaysReferenceSrv,
            accessManager, errorResponseManager, authHeaderManager, logoutManager, expirationManager, redirectManager) {

            console.log("PastDaysReferenceCtrl loaded with action: " + $params.action)
            ...
            */
            ...
            if (['edit'].indexOf($params.action) == -1) {
                ...
                $location.path("/home/pastDaysReferences");
            }

            function find(id) {
                pastDaysReferenceSrv.find(id, function (error, data) {
                    if (error) {
                        console.log(error);
            ...
            $scope.modify = function () {
                pastDaysReferenceSrv.modify($scope.data, function (error, data) {
                    if (error) {
                        console.log(error);
                        errorResponseManager.checkResponse(error);
                        return;
