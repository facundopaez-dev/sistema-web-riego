	...
	["$scope", "$location", "$route", "HarvestSrv", "ParcelSrv", "CropSrv", "ReasonError", "UtilDate", "AccessManager", "ErrorResponseManager", "AuthHeaderManager",
		...
		function ($scope, $location, $route, harvestSrv, parcelSrv, cropSrv, reasonError, utilDate, accessManager, errorResponseManager, authHeaderManager,
			...
			*/
			...
			function findAll() {
				harvestSrv.findAll(function (error, data) {
					if (error) {
			...
			$scope.delete = function (id) {
				...
				harvestSrv.delete(id, function (error) {
					if (error) {
						console.log(error);
						errorResponseManager.checkResponse(error);
						return;
			...
			$scope.filter = function () {
				...
				fecha para filtrar datos. Por lo tanto, la aplicacion muestra
				...
				*/
				...
				de una parcela para filtrar datos. Por lo tanto, la aplicacion
				...
				*/
				...
				harvestSrv.filter($scope.parcel.name, newDate, function (error, data) {
					if (error) {
						console.log(error);
						errorResponseManager.checkResponse(error);
						return;
